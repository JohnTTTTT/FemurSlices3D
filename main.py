...
def buildTriangles( slice0, slice1 ):
    ...
    for v1 in slice0.verts:
        for v2 in slice1.verts:
            ...
            if d < minDistance[0]:             
            #If the distance is smaller than the minDistance, re initialize the min
                minDistance = [d,v1,v2]
    ...
    #Initialize the point of slice 0
    #Initialize the permutation of slice 0 by adding the vertices before and after the index

    point1 = slice1.verts.index(minDistance[2])  
    permutation1 = slice1.verts[point1:] + slice1.verts[:point1] + [minDistance[2]]
    #Initialize the point of slice 1
    #Initialize the permutation of slice 1 by adding the vertices before and after the index

    length0 = len(permutation0)
    length1 = len(permutation1)
    #Intialize the amount of elements in permutation0 and permutation1
    ...
    for r in range(length1):
        minArea.append([])
        minDir.append([])   
        for c in range(length0):         
            minArea[r].append([])
            minDir[r].append([])
            #Initialize the arrays of both minArea and minDir
    ...
    for r in range(1,length0):    
        minDir[0][r] = "-"
        minArea[0][r] = minArea[0][r-1] + triangleArea(permutation1[0].coords, permutation0[r].coords, permutation0[r-1].coords) 
        #First row of both arrays are special cases, initialized both accordingly
    ...
    for c in range(1,length1): 
        minDir[c][0] = "|"
        minArea[c][0] = minArea[c-1][0] + triangleArea(permutation0[0].coords, permutation1[c].coords, permutation1[c-1].coords)
        #First column of both arrays are special cases, initialized both accordingly
        #Ignoring base case since it's different (".")
    ...
    for r in range(1,length1):
        for c in range(1,length0):
            Area0 = minArea[r-1][c] + triangleArea(permutation0[c].coords, permutation1[r].coords, permutation1[r-1].coords)
            Area1 = minArea[r][c-1] + triangleArea(permutation1[r].coords, permutation0[c].coords, permutation0[c-1].coords)
            if Area0 < Area1:                      
                minDir[r][c] = "|"
                minArea[r][c] = Area0
            else:
                if Area0 >= Area1:                 
                    minDir[r][c] = "-"
                    minArea[r][c] = Area1
            #Fill in remaining slots of the minDir and minArea, whichever area is smaller between Area0 and Area1 will be inserted inside minArea
    ...
    x = length0-1        
    y = length1-1 
    #Since we want to account for index 0        

    while True:
        if x is 0 and y is 0:
        #If we have the zero index that means we are finished the list of triangles, exit the loop
            break
        else:
            if minDir[y][x] == "|": 
            #If minDir is "|" we traverse upwards on the column
                triangles.append(Triangle([permutation0[x],permutation1[y],permutation1[y-1]]))
                y-=1
            else:
                if minDir[y][x] == "-":
                #If minDir is "-" we traverse left on the row  
